use egui::{Align, Align2, RichText, Stroke, Vec2b};
...
fn render_type(
    ...
    if let (Some(name), Some(memory)) = (&variable.name, &variable.memory) {
        ...
        position.x += BAR_THICKNESS * (multiplier <= 1.5) as u32 as f64 + ADDR_LENGTH as f64 + BAR_THICKNESS * offset as f64 + offset as f64 * BAR_PADDING;
        ...
        match &variable.types.0[type_index].1 {
            stackium_shared::TypeName::Name { name: _, byte_size: _ } => {}
            ...
            stackium_shared::TypeName::Ref { index: _ } => {
                ...
                ui.arrows(Arrows::new(
                    PlotPoints::new(vec![[position.x, position.y]]),
                    PlotPoints::new(vec![[ptr_dst.x, ptr_dst.y]]),
                ).tip_length(5.0f32));
            }
    ...
    fn ui(&mut self, ui: &mut egui::Ui) -> (bool, egui::Response) {
            ...
            (self.variables.ready(), self.registers.ready())
        {
            ...
            if self.cached_addresses.is_none() {
                let mut addresses = variables
                    .iter()
                    .map(|v| {
                        (v.addr.unwrap() - VARIABLE_MEM_PADDING)
                            ..(v.addr.unwrap() - VARIABLE_MEM_PADDING + v.memory.as_ref().unwrap().len() as u64)
                    })
                    .flatten()
                    .collect::<HashSet<_>>()
                    .into_iter()
